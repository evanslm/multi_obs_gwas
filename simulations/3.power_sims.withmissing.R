...
params<-as.numeric(args[1]) ### Parameter combination number. 29-32 have missingness included
...
nmiss<-param.combins[['missingness']][params]
...
U<-foreach(nn=1:1000,.combine=rbind,.errorhandling="remove")%dopar%{
	...
	if(nmiss>0) y<-t(sapply(1:n,make.missing,y=y,nwaves=nwaves,nmiss=nmiss))
	...
	c(nsig,bcor,bcor.pairs,r2single,r2mean,r2med)
}
colnames(U)<-c(paste0("nsig_",traits),paste0("nsig_",traits,".null"),paste0(traits,"_corb"),paste0(traits[trait.pairs[,1]],collapse="_"),paste0(traits[trait.pairs[,2]],collapse="_"),paste0(traits[trait.pairs[,3]],collapse="_"),paste0(traits,"_prsr2"))

summary(U)
simresults$U<-U
save(simresults,file=paste0("results/sims",params,".RDat"))
